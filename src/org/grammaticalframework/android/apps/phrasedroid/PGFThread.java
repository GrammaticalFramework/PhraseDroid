...
class PGFThread extends Thread {
    ...
    /** 
     * @param activity the activity that started the thread
     *        and on which the results will be displayed.
     * @param sLang the source language
     * @param tLang the target language
     **/
    public PGFThread(Context context,
                     String sLang,
                     String tLang)
    {
	...
	InputStream is =
	    mContext.getResources().openRawResource(phrasebook_resource);
	final long begin_time = System.currentTimeMillis();
	try {
	    this.mPGF = PGFBuilder.fromInputStream(is, new String[] {sLang, tLang});
	} catch (IOException e) {
    ...
    // The pgf handler is the main class of the thread.
    // It will handle operation on and with the pgf file.
